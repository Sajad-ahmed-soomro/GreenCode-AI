...
declare const fetch: (input: string, init?: { method?: string; headers?: Record<string, string>; body?: string }) => Promise<{ ok: boolean; status: number; text(): Promise<string>; json(): Promise<unknown> }>;

const DIAGNOSTIC_COLLECTION = 'greencode';
let diagnosticCollection: vscode.DiagnosticCollection;
let statusBarItem: vscode.StatusBarItem;
let energySummaryStatusBarItem: vscode.StatusBarItem;
let highCriticalDecoration: vscode.TextEditorDecorationType;
let mediumCriticalDecoration: vscode.TextEditorDecorationType;
let lowCriticalDecoration: vscode.TextEditorDecorationType;
let highEnergyDecoration: vscode.TextEditorDecorationType;
let classSummaryDecoration: vscode.TextEditorDecorationType;
let codeLensEmitter: vscode.EventEmitter<void>;
let lastAnalysis: { issues: any[]; patches: any[] } = { issues: [], patches: [] };
const analysesByFile = new Map<string, { issues: UnifiedIssue[]; patches: any[] }>();
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    highCriticalDecoration,
    mediumCriticalDecoration,
    lowCriticalDecoration,
    highEnergyDecoration,
    classSummaryDecoration
  );

  const config = () => vscode.workspace.getConfiguration('greencode');
  const gatewayUrl = () => (config().get<string>('gatewayUrl') || 'http://localhost:5400').replace(/\/$/, '');
  ...
  const applyFixCommand = vscode.commands.registerCommand('greencode.applyFix', async (issueId?: string) => {
    if (issueId) {
      await applyFixById(issueId);
    } else {
      const editor = vscode.window.activeTextEditor;
      if (editor) await applyFixAtCursor(editor);
        else vscode.window.showErrorMessage('No active editor');
    }
  });

  const showEnergyBreakdownCommand = vscode.commands.registerCommand(
    'greencode.showEnergyBreakdown',
    async (summary: string) => {
      vscode.window.showInformationMessage(summary);
    }
  );

  const analyzeOnSave = vscode.workspace.onDidSaveTextDocument(async (doc: vscode.TextDocument) => {
    if (!config().get<boolean>('analyzeOnSave') || doc.languageId !== 'java') return;
  ...
  const onVisibleEditorsChanged = vscode.window.onDidChangeVisibleTextEditors((editors) => {
    ...
    if (!active || active.document.languageId !== 'java') return;
    if (!editors.some((e) => e.document.uri.toString() === active.document.uri.toString())) return;
    refreshDecorationsForActiveEditor();
  });
    ...
    { language: 'java' },
    {
      provideCodeActions(document: vscode.TextDocument, range: vscode.Range, context: vscode.CodeActionContext) {
        ...
        for (const diag of context.diagnostics) {
          ...
          const issueId = (diag as any).issueId as string | undefined;
          if (!issueId) continue;
          const patch = lastAnalysis.patches.find((p: any) => p.issueId === issueId);
          if (patch && patch.recommendation) {
            const action = new vscode.CodeAction(
              `GreenCode: ${patch.recommendation.slice(0, 50)}...`,
  ...
  const hoverProvider = vscode.languages.registerHoverProvider(
    { scheme: 'file', language: 'java' },
    {
      async provideHover(document: vscode.TextDocument, position: vscode.Position) {
        ...
        const issues = lastAnalysis.issues.filter(
          (i: UnifiedIssue) => i.filePath === filePath && (i.line === line || i.line === line + 1)
        );
        ...
        for (const issue of issues) {
          ...
          if (issue.explanation) md.appendMarkdown(`  ♻️ ${issue.explanation}\n`);
          if (issue.recommendation) md.appendMarkdown(`  ✅ ${issue.recommendation}\n`);
        }
  ...
  const codeLensProvider: vscode.CodeLensProvider = {
    ...
    async provideCodeLenses(document: vscode.TextDocument): Promise<vscode.CodeLens[]> {
      ...
      const functionSymbols = flattenSymbols(symbols).filter(
        (s) => s.kind === vscode.SymbolKind.Method || s.kind === vscode.SymbolKind.Function
      );
      const classSymbols = flattenSymbols(symbols).filter(
      ...
      for (const cls of classSymbols) {
        const issuesInClass = fileIssues.filter((issue) => {
          ...
          if (!isEnergyIssue) return false;
          const line = Math.max(0, issue.line || 0);
          return line >= cls.range.start.line && line <= cls.range.end.line;
        });
        if (issuesInClass.length === 0) continue;
        const totals = issuesInClass.reduce(
          (acc, issue) => {
        ...
        lenses.push(
          new vscode.CodeLens(new vscode.Range(cls.range.start, cls.range.start), {
            title: classTitle,
      ...
      for (const fn of functionSymbols) {
        ...
        if (issuesInFn.length === 0) continue;

        const total = issuesInFn.reduce(
          (acc, issue) => {
            const e = estimateEnergyImpact(issue);
            acc.mWh += e.mWhPer1kRuns;
            acc.co2 += e.co2eMg;
        ...
        const details = issuesInFn
          ...
          .join(' | ');
        lenses.push(
          new vscode.CodeLens(new vscode.Range(fn.range.start, fn.range.start), {
            title,
            command: 'greencode.showEnergyBreakdown',
...
async function runAnalysisForDocument(document: vscode.TextDocument, gatewayUrl: string) {
  ...
  try {
    ...
    const data = (await res.json()) as { unifiedIssues?: unknown[]; patches?: unknown[] };
    const issues: UnifiedIssue[] = (data.unifiedIssues || []) as UnifiedIssue[];
    const patches: any[] = data.patches || [];
    lastAnalysis = { issues, patches };
    analysesByFile.set(fileName, { issues, patches });
    codeLensEmitter.fire();

    const severityThreshold = vscode.workspace.getConfiguration('greencode').get<string>('severityThreshold') || 'medium';
    const order = { high: 0, medium: 1, low: 2 };
    const threshold = order[severityThreshold as keyof typeof order] ?? 1;
    const filtered = issues.filter((i: UnifiedIssue) => (order[i.severity as keyof typeof order] ?? 2) <= threshold);
    ...
    for (const issue of filtered) {
      ...
      (diag as any).issueId = issue.id;
      if (issue.recommendation) diag.code = issue.recommendation;
    ...
    if (isNetwork) {
      vscode.window.showErrorMessage(
        `GreenCode AI: Cannot reach the gateway at ${gatewayUrl}. Start it with: cd modules/gateway && npm start`,
        'Open settings'
      ).then((choice) => {
...
async function applyFixById(issueId: string) {
  const patch = lastAnalysis.patches.find((p: any) => p.issueId === issueId);
  if (!patch) {
  ...
  await editor.edit((eb: vscode.TextEditorEdit) => eb.replace(range, replacement));
  vscode.window.showInformationMessage('Fix applied.');
}

async function applyFixAtCursor(editor: vscode.TextEditor) {
  const line = editor.selection.active.line;
  const filePath = path.basename(editor.document.uri.fsPath);
  const patch = lastAnalysis.patches.find(
    (p: any) => p.filePath === filePath && (p.line === line || p.line === line + 1)
  );
  if (patch) await applyFixById(patch.issueId);
  else vscode.window.showInformationMessage('No fix available at cursor. Run analysis first.');
}
...
function updatePinnedEnergySummary(fileName: string, issues: UnifiedIssue[]): void {
  ...
  const totals = energyIssues.reduce(
    (acc, issue) => {
      const e = estimateEnergyImpact(issue);
      acc.mWh += e.mWhPer1kRuns;
      acc.co2 += e.co2eMg;
...
function buildEnergyInterpretation(issue: UnifiedIssue): string {
  const isEnergyIssue = issue.category === 'energy' || issue.agent === 'energy';
  if (!isEnergyIssue) {
...
function applyCriticalityDecorations(document: vscode.TextDocument, issues: UnifiedIssue[]): void {
  ...
  for (const issue of fileIssues) {
    ...
    if (issueLine >= document.lineCount) continue;
    ...
    const hover = new vscode.MarkdownString(
      `**${getSeverityIcon(issue.severity)} ${issue.agent || 'agent'}**\n\n` +
    ...
    if (issue.severity === 'high') highRanges.push(opt);
...
function applyClassSummaryDecorations(document: vscode.TextDocument, issues: UnifiedIssue[]): void {
  const editor = vscode.window.visibleTextEditors.find((e) => e.document.uri.toString() === document.uri.toString());
  if (!editor) return;
  const fileName = path.basename(document.uri.fsPath);
  const fileIssues = issues.filter((i) => (i.filePath || fileName) === fileName);
  ...
  const classLines: Array<{ line: number; name: string }> = [];
  for (let i = 0; i < document.lineCount; i++) {
    const text = document.lineAt(i).text;
    const m = text.match(classRegex);
    if (m?.[3]) classLines.push({ line: i, name: m[3] });
  }
  ...
  const classDecorations: vscode.DecorationOptions[] = [];
...
function estimateEnergyImpact(issue: UnifiedIssue): { mWhPer1kRuns: number; co2eMg: number } {
  ...
  if (issue.agent === 'energy' || issue.category === 'energy') {
    ...
    if (typeof energyScore === 'number' && Number.isFinite(energyScore)) {
      ...
      const mWhPer1kRuns = Math.round(
        Math.max(0.1, energyScore * 18 * confidence * workloadFactor * complexityFactor * runFactor) * 10
      ) / 10;
...
function buildInlineEnergyLabel(issue: UnifiedIssue): string {
  const e = estimateEnergyImpact(issue);
