...
function isConstantField(fieldName, lineText) {
    ...
    return commonConstantPatterns.some(pattern => fieldName.toUpperCase().startsWith(pattern));
}
// Helper to detect if a method is a getter/setter
function isGetterSetterMethod(methodName) {
...
class ComplianceAgent {
    ...
    analyze(code, ast) {
        ...
        if (ast.functions && Array.isArray(ast.functions)) {
            const fnRule = rules_js_1.namingRules.find(r => r.name === "method_camel_case"); // Updated
            ast.functions.forEach(func => {
                ...
                if (!fnRule.pattern.test(fname)) {
                    ...
                    suggestionsList.push({
                        line,
                        suggestion: suggestions_js_1.suggestions.suggestCamelCase(fname) // Updated
                    });
                }
            });
        }
        ...
        if (ast.classes && Array.isArray(ast.classes)) {
            ...
            const methodRule = rules_js_1.namingRules.find(r => r.name === "method_camel_case"); // Updated
            const fieldRule = rules_js_1.namingRules.find(r => r.name === "variable_camel_case"); // Updated
            const constantRule = rules_js_1.namingRules.find(r => r.name === "constant_upper_snake_case"); // Added
                ...
                if (cls.methods && Array.isArray(cls.methods)) {
                    cls.methods.forEach(method => {
                        ...
                        if (methodType.isGetter || methodType.isSetter || methodType.isBooleanGetter) {
                            ...
                            if (baseName.length === 0 || !/^[A-Z]/.test(baseName)) {
                                ...
                                if (methodType.isGetter) {
                                    suggestionsList.push({
                                        line,
                                        suggestion: suggestions_js_1.suggestions.suggestGetterMethodName(baseName.toLowerCase())
                                    });
                                }
                                else if (methodType.isSetter) {
                                    suggestionsList.push({
                                        line,
                                        suggestion: suggestions_js_1.suggestions.suggestSetterMethodName(baseName.toLowerCase())
                                    });
                                }
                                else if (methodType.isBooleanGetter) {
                                    suggestionsList.push({
                                        line,
                                        suggestion: suggestions_js_1.suggestions.suggestBooleanGetterMethodName(mname)
                                    });
                                }
                            }
                        }
                        // Regular method naming check
                            ...
                            suggestionsList.push({
                                line,
                                suggestion: suggestions_js_1.suggestions.suggestCamelCase(mname) // Updated
                            });
                        }
                    });
                }
                    ...
                    cls.fields.forEach(field => {
                        ...
                        if (isConstantField(fname, lineText)) {
                            ...
                            if (constantRule && !constantRule.pattern.test(fname)) {
                                ...
                                suggestionsList.push({
                                    line,
                                    suggestion: suggestions_js_1.suggestions.suggestUpperSnakeCase(fname) // Updated
                                });
                            }
                        }
                        else {
            ...
            if (lineText.trim().startsWith('import ')) {
                ...
                if (importMatch) {
                    ...
                    if (!isUsed) {
                        ...
                        suggestionsList.push({
                            line: lineNumber,
                            suggestion: suggestions_js_1.suggestions.suggestRemoveUnusedImport(importName)
                        });
                    }
                }
            }
            ...
            if (lineText.trim().startsWith('package ')) {
                ...
                if (packageMatch) {
                    ...
                    if (packageName !== packageName.toLowerCase()) {
                        ...
                        suggestionsList.push({
                            line: lineNumber,
                            suggestion: suggestions_js_1.suggestions.suggestPackageNaming(packageName)
                        });
                    }
                }
            }
