export default class DataStructureAgent {
    ...
    // STEP 2.1 — Load Java + AST
       loadInput(sourceCode: string, astObject: any): void {
...
analyze(): void {
    this.suggestions.push(
        ...
        // G category
        ...this.checkBigTemporaryList() ,
          ...this.checkRepeatedListToSetConversion(),
        ...this.checkRepeatedListToMapConversion()     // G1
    );
}
 
checkRepeatedListToSetConversion(): any[] {
    const suggestions: any[] = [];
    if (!this.sourceCode) return suggestions;
...
checkRepeatedListToMapConversion(): any[] {
    const suggestions: any[] = [];
    if (!this.sourceCode) return suggestions;
...
checkSortingInsideLoop(): any[] {
  ...
  checkBigListNeverIndexed(): any[] {
    const suggestions: any[] = [];
    if (!this.sourceCode) return suggestions;

    const lines = this.sourceCode.split("\n");

    const listDeclared: Map<string, number> = new Map(); // varName → line number
    const listIndexed: Set<string> = new Set();
    const listForeachUsed: Set<string> = new Set();
...
checkDuplicateDataStructures(): any[] {
    const suggestions: any[] = [];
    if (!this.sourceCode) return suggestions;
  ...
  // RULE CATEGORY D/E — Memory issues & wrong data usage
    checkMemoryIssues(): any[] {
        // Empty for now
        return [];
    }

    // RULE CATEGORY H — Repeated conversions (list→set)
    checkRepeatedConversions(): any[] {
        // Empty for now
        return [];
    }
checkArraySearchedByKey(): any[] {
    const suggestions: any[] = [];
    if (!this.sourceCode) return suggestions;
...
checkDictionaryOrderMisuse(): any[] {
    const suggestions: any[] = [];
    if (!this.sourceCode) return suggestions;
