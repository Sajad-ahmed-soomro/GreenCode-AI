...
import fs from "fs";
import { runAgentsParallel } from "../../orchestrator/run-agents-parallel.js";
import { normalizeFromScanOutput } from "../../orchestrator/index.js";
import { createContextMemory } from "../../integrations/context-memory/index.js";
import { runRefactoringAdapter } from "../../integrations/refactoring-adapter/index.js";
...
export default function runAnalysisPipeline(projectPath, scanOutputDir, res) {
  return new Promise((resolve, reject) => {
    ...
    analyzer.on("close", async (code) => {
      ...
      await validateStaticAnalyzer();
    });

    const validateStaticAnalyzer = async () => {
      ...
      // ---- Run all 4 agents in parallel (orchestrator) ----
      let agentCounts = { dataStructure: 0, optimization: 0, maintainability: 0, compliance: 0 };
      try {
        agentCounts = await runAgentsParallel(projectPath, scanOutputDir);
        console.log("📊 Orchestrator: dataStructure=%d optimization=%d maintainability=%d compliance=%d",
          agentCounts.dataStructure, agentCounts.optimization, agentCounts.maintainability, agentCounts.compliance);
      } catch (err) {
...
// 4. Call runEnergyAnalyzer; then augment response with unified issues, context memory, and refactoring patches
      const contextMemory = createContextMemory();
      const onEnergyDone = async (data) => {
        try {
          const projectId = scanId;
          const issues = normalizeFromScanOutput(scanOutputDir, projectId);
          await contextMemory.storeAnalysis(projectId, { issues, scanId, timestamp: new Date().toISOString() });
          const { patches } = await runRefactoringAdapter(issues, { useGoEngine: false });
          data.unifiedIssues = issues;
          data.patches = patches;
          data.summary = data.summary || {};
          data.summary.unifiedIssueCount = issues.length;
          data.summary.patchCount = patches.length;
        } catch (e) {
          console.warn("Post-process (unified/context/refactor):", e.message);
        }
        sendResponse(data);
      };
      runEnergyAnalyzer(
        ...
        onEnergyDone,
        {
          runBenchmarks: javaFilesCount > 0,
          sourceCodePath: javaFilesCount > 0 ? javaSourceDir : null,
          mockBenchmarks: javaFilesCount === 0
        }
      );
    };
