...
const MethodCard = ({ method, isSelected, onSelect }) => {
  // Use normalized method data
  const energyScore = method.energyScore || 0;
  const cpuScore = method.cpuScore || 0;
  const memScore = method.memScore || 0;
  const metrics = method.metrics || {};
  
  const getEnergyLevel = (score) => {
    if (score > 0.6) return { label: "High", color: "bg-red-100 text-red-800 border-red-200", icon: <AlertCircle className="w-4 h-4" /> };
      ...
      <div className="p-6">
        <div className="grid grid-cols-4 gap-3 mb-4">
          ...
          <div className="text-center p-3 bg-blue-50 rounded-lg border border-blue-100">
            ...
            <p className="text-xl font-bold text-blue-700">{cpuScore.toFixed(2)}</p>
          </div>
          <div className="text-center p-3 bg-orange-50 rounded-lg border border-orange-100">
            <p className="text-xs text-orange-600 mb-1">Memory</p>
            <p className="text-xl font-bold text-orange-700">{memScore.toFixed(2)}</p>
          </div>
          <div className="text-center p-3 bg-teal-50 rounded-lg border border-teal-100">
            ...
            <p className="text-xl font-bold text-teal-700">
              {metrics.efficiencyScore ? metrics.efficiencyScore.toFixed(2) : '0.00'}
            </p>
          </div>
        </div>
